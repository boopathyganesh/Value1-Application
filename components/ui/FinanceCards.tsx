...
const FinanceCard = ({ title, content, image}: CardProps) => {
    return (
        <div className='flex flex-col items-center justify-between w-72 h-52 bg-[#778899] rounded-2xl overflow-hidden'>
            <div className='flex flex-col items-start justify-center gap-1.5 text-white p-5'>
                <div className='flex items-center justify-center gap-4'>
                    ...
                    <h1 className='text-lg font-medium w-44'>{title}</h1>
                </div>
                <p className='text-gray-500 text-sm'>{content}</p>
                <a href="" className='mt-3 p-2 bg-black rounded-lg'>Apply Now</a>
            </div>
        </div>
    )
}
